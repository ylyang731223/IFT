...
df = df[['smiles', 'P(Mpa)', 'T (R )', 'IFT(mN/m)']]
print(df.head())

smile_data_x = df.values[:,0]
numerical_data_x = df.values[:,1:3].astype(float)
y = df.values[:,3]
...
z = Embedding(output_dim=512, input_dim=len(tokenizer.word_index)+1, input_length=max_length)(nlp_input)
...
results = model.evaluate({'nlp_input': X[:,:max_length], 'meta_input': X[:,max_length:]},{'main_output': y}, batch_size=32)
print(results)
